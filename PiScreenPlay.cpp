...
void piScreenCls::playF(uint8_t x, uint8_t ad){
	if (!x) {
...
void mainThreadCls::clsExMes::strExec::psPlayF(uint8_t x, uint8_t ad) {
	piScreenC.playF(x, ad);
}

void piScreenCls::playPrep(uint16_t x, uint8_t ad) {
...
void piScreenCls::playP(uint8_t x, uint8_t ad) {
	...
	while (strbasprc::charPLen(strbasprc::charPLsR(lsPlay + 1, fileObj.file, fileObj.loc))) {
		uint8_t adSnd = (lsPlay[3] - '0') * 10 + (lsPlay[4] - '0'), b;
