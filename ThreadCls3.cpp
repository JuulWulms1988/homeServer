...
void mainThreadCls::clsExMes::naCheckF(strSndV& s, unsigned __int8 ad, unsigned __int8 ad2) {
	strMpCh& Po = threadCls->naCh2Mp[ad2][ad];
	if (!Po.begin(s)) return;

	threadBufCls::recStruct{
		threadCls->threadMut[ad2], [](void* pMCh, void* ad) {
		strMpCh& Po = threadCls->naChckMpCh[(int)ad];
		do {
			if (Po.begin(((strMpCh*)pMCh)->p)) 
				threadBufCls::recStruct{
				threadCls->threadMut[(int)ad], [](void* pMCh, void* p) {
				do ((strExec*)p)->naCheck(((strMpCh*)pMCh)->p, ((strMpCh*)pMCh)->ad); while (!((strMpCh*)pMCh)->end());
				} }.start(&Po, &threadCls->uitEx[(int)ad]);
		} while (!((strMpCh*)pMCh)->end());
	} }.start(&Po, (void*)ad);
}


void mainThreadCls::clsExMes::strExec::naCheck(strSndV& s, unsigned __int8 ad) {
...
void mainThreadCls::statStrucStr::start(stSndStt& st, unsigned __int8 ad) { 
	mut.lock(); if (!ready) {
...
void mainThreadCls::stSndStt::funcSnd() {
	char* ls = s.pack; int& len = s.len; unsigned __int8 strAd = (ls[2] - '0') * 10 + (ls[3] - '0');
	for (unsigned __int8 t = 0, m = len; t < m; t++) ls[m - t] = ls[m - 1 - t]; len++; ls[len++] = '+', *ls = '@', ls[1] = 'U', ls[2] = 'V';
	if (type == 3) {
...
void mainThreadCls::stSndStt::funcSnd2() {
	char* ls = s.pack; int& len = s.len; unsigned __int8 strAd = (ls[3] - '0') * 10 + (ls[4] - '0');
	if (type == 5) {
		unsigned int wW = threadCls->statDimVar[no].load(); ls[5] = strbasprc::cv16char0(wW), ls[6] = strbasprc::cv16char1(wW);
	}
	else if (threadCls->statVar[no].load()) ls[5] = 'A', ls[6] = 'N'; else ls[5] = 'U', ls[6] = 'T';
	unsigned __int8 ad2 = (ls[len] - '0') * 60 + (ls[len + 1] - '0');
...
void mainThreadCls::stSndStt::func() {
	...
	char* buf; char* ls; int sendLen; unsigned __int8 bufX, strAd; __int8 lsNow = p->getStr(ls), bfNow = p->getStr(buf); bool bf = false;
	...
	while (!strbasprc::vergCharP(strbasprc::remCommCharP(strbasprc::charPLsR(buf, fl, loc)), "GG")) {
		if (buf[0] == 'P' && buf[1] == 'S') {
			sluisPiScreenF([&] {
				unsigned __int16 V = strbasprc::cvintcharint(buf[2]) * 60 + strbasprc::cvintcharint(buf[3]);
				if (val) V += 512; else V += 256; return V;
			}(), strAd);
			continue;
		} lsNow = p->getStr(ls);
		unsigned __int8 strAd2 = (buf[2] - '0') * 10 + (buf[3] - '0');
		if (buf[0] == 'S' && buf[1] == 'P') {
			if (!val) ledPlF(ls, lsNow, strAd2);
			else if (lsNow != 100) p->retStr(lsNow);
			else delete[32] ls; continue;
		...
		else if (lsNow != 100) p->retStr(lsNow); else delete[32] ls;
	} s.ret();
	if (bfNow != 100) p->retStr(bfNow); else delete[32] buf;
}
