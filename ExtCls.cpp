...
void extCls::begin() {
	do {
		ifstream hF; hF.open("hiber\\hiber.txt"); if (!hF.is_open()) return;
		hF.seekg(-2, ios::end);
		{
			bool flag = false;
			for (unsigned __int8 t = 0; t < 2; t++) if (hF.get() != 'z') { flag = true; break; }
			if (flag) { cout << "weg\n"; hF.close(); break; }
		}
		hF.seekg(0, ios::beg);
		...
		}
		{
			...
			if ((*read(ls, hF) == 'z' && ls[1] == 'z') || !(p = p_ledPl.toObjF(strbasprc::cvintcharStr(ls))) || hF.eof() || strbasprc::cvintcharStr(read(ls, hF)) != p->stripCnt || hF.eof()) break;
			{
				...
				for (unsigned __int8 t = 0; (hF.eof() && !(flag = true)) || t < 5; t++, in = strbasprc::cvintcharStr(read(ls, hF))) switch (t) {
				...
				p->upS = in;
			} if (!p->stripCnt) continue; bool* gezet = p->gezet; for (unsigned __int8 t = 0, m = p->stripCnt; t < m; t++) gezet[t] = false;
			for (unsigned __int8 tt = 0, mm = p->stripCnt; (flag && !flag) || (tt < mm && (gezet[tt] = true)); gezet[tt++] = false)
			{
				p->var[tt].parChld = strbasprc::cvintcharStr(read(ls, hF));
				for (unsigned __int8 t = 0, m = p->stripCnt; t < m; t++) p->var[tt].parChldAr[t] = strbasprc::cvintcharStr(read(ls, hF)) * true;
				for (unsigned __int8 t = 0, m = LEDPL_VARAANT2 + 40; ((tt || t) && hF.eof() && !(flag = true)) || (t == LEDPL_VARAANT && (t = 20)) || (t == LEDPL_VARAANT1 + 20 && (t = 40)) || t < m; t++, l = 0)
					p->setVal(t, strbasprc::cvintcharStr(read(ls, hF)), 1);
			}
			if (flag) break;
		} while (true);
		for (unsigned __int8 t = 0; t < 4; t++) piSWrite(t, read(ls, hF));
		piSPlay(1 + (*read(ls, hF) - '0'));
		unique_lock<shared_mutex>(radioS.varMut), radioS.chan = (*read(ls, hF) - '0') * 100 + (ls[1] - '0') * 10 + (ls[2] - '0');
		hF.close();
	} while (false); 
	system("del hiber\\hiber.txt /Q /F");
}
...
void extCls::end() {
	do {
		ifstream hF; hF.open("hiber\\hiber.txt"); if (!hF.is_open()) break;
		hF.close();
		system("del hiber\\hiber.txt /Q /F");
	} while (false);
	ofstream hF;
	hF.open("hiber\\hiber.txt");
	{
		char w[3] = { 'g', '\n', '\0' };
		atomic<bool>* p = threadCls->statVar;
	...
	{
		char w[4]; unsigned __int8 l = 0;
		atomic<int>* p = threadCls->statDimVar;
		for (unsigned int t = 0; t < 900; t++, l = 0) w[(l += strbasprc::cvcharlngth(60, p[t].load(), w))++] = '\n', w[l] = '\0', hF << w;
	}
	for (map<unsigned __int8, mainThreadCls::clsLedPl::clsLedPlObj&>::iterator t = p_ledPl.mpObj.begin(), m = p_ledPl.mpObj.end(); t != m; t++) {
		...
		for (unsigned __int8 tt = 0, mm = p.stripCnt; tt < mm; tt++)
		{
			w[(l += strbasprc::cvcharlngth(60, p.var[tt].parChld, w))++] = '\n', w[l] = '\0', l = 0, hF << w;
			for (unsigned __int8 t = 0, m = p.stripCnt; t < m; t++) w[0] = [&] { if (p.var[tt].parChldAr[t]) return '1'; return '0'; }(), w[1] = '\n', w[2] = '\0', hF << w;
			for (unsigned __int8 t = 0, m = LEDPL_VARAANT2 + 40; (t == LEDPL_VARAANT && (t = 20)) || (t == LEDPL_VARAANT1 + 20 && (t = 40)) || t < m; t++, l = 0)
	...
	hF << "zz\n";
	{
		...
		for (unsigned __int8 t = 0; t < 5 && ((p = piSRead(t)) || !p); t++, l = 0)
			switch (t) {
			case 0: *w = strbasprc::cvintcharchar((l = (int)(p)) / 60), w[1] = strbasprc::cvintcharchar(l % 60); w[2] = '\n'; w[3] = '\0'; hF << w; break;
			case 1: if (p) *w = '1'; else *w = '0'; w[1] = '\n'; w[2] = '\0'; hF << w; break;
			case 2: *w = strbasprc::cvintcharchar((l = (int)(p)) / 60), w[1] = strbasprc::cvintcharchar(l % 60); w[2] = '\n'; w[3] = '\0'; hF << w; break;
			case 3: { char* P = w; char* T = (char*)p; while (*T) *(P++) = *(T++); *(P++) = '\n'; *(P++) = '\0'; hF << w; break; }
			}
	}
	hF << (char)((piSPlay(0) * 1) + '0') << '\n';
	{   
		unsigned __int8 C; shared_lock<shared_mutex>(radioS.varMut), C = radioS.chan;
