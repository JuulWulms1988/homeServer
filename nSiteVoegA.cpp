...
void clsNSite::func::voegAN(bool x) {
	...
	if (fileObj.open(buf)) {
		char* c = fileObj.file;
		voegC('N'); for (__int8 t = 0; t < 3; t++) voegC(*c++);
	} voeg("\', ", 3); if (x) voeg("true", 4); else voeg("false", 5);
}

void clsNSite::func::voegAD(unsigned __int8 x) {
	buf[4] = 'N', buf[5] = 'O';
	bufLen -= 2;
	strncpy_s(buf + 21, bufLen + 1, buf + 2, bufLen);
	strncpy_s(buf, 21, "Apparaten\\00\\Nummers", 20);
	...
	if (fileObj.open(buf)) {
		char* c = fileObj.file;
		voegC('D'); for (__int8 t = 0; t < 3; t++) voegC(*c++);
	} voeg("\', ", 3); { bool al = false; for (unsigned __int8 t = 100, r; t; t /= 10) if ((r = x / t % 10) || al || t == 1) al = true, voegC(r + '0'); }
}
...
void clsNSite::func::stwzF(char* nGet, int nLen) {
	...
	char * B; __int8 Now; bool oTel = true;
	if (!(*nGet - '0')) {
		oTel = false;
		if (nGet[4] == '0') {
	...
	pE.exCls.voeg(mainThreadCls::clsExMes::var{ B, Now, (unsigned __int8) bufLen, -(2 + oTel * 1) });
}
