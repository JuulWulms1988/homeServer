...
void mainThreadCls::clsLedPl::clsLedPlObj::atDetFunc(char* xLs, int xLen) {
	...
	uint8_t strNo = (xLs[xLen - 2] - '0') * 10 + (xLs[xLen - 1] - '0'), clNo; if (atDetBl) clNo = (xLs[1] - '0') * 10 + (xLs[2] - '0');
	{
		varMut.lock();
		varStr& p = var[strNo]; bool* pAr = p.parChldAr;
		auto detInFunc = [&] {
			for (uint8_t tt, t = tt = 0, m = p.parChld; tt < m; t++)
		...
		if (!atDetBl) 
			...
			for (uint8_t t = 0; ((t == clNo || t == strNo) && (((++t == clNo || t == clNo) && ++t < stripCnt) || t < stripCnt)) || t < stripCnt; t++)
			...
			for (uint8_t tel = 0; tel < stripCnt; tel++) if (pAr0[tel] && tel != strNo) { clNo = tel; break; }
