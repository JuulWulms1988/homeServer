...
mainThreadCls::clsExMes::clsExMes(trLisHomeCls* pp) {
	{
		...
		for (__int8 t = 0, z = 99; t < 99; t++) lsA[t] = z--;

		
		for (__int8 t = 0; t < 20; t++) exCls.set(t, t);
		char* jj;
		{ __int8 t = 1; strExec* y = exCls.i; while ((t - 1 && t < 21 && y++) || t == 1) y->set(this, getStr(jj)), t++; }
	}
	cvEx.ready = true;
}
...
void printTijdel(char** p, unsigned int l) {
	...
	thread ([]() {
		do {
			int L = printMap.p.len; char ** PP = (char**)strbasprc::cvintcharStr(printMap.p.pack); delete[32] printMap.p.pack;
			for (unsigned int T = 0; T < L; T++)
				cout << PP[T];
			for (unsigned int T = 0; T < L; T++) delete[strbasprc::charPLen(PP[T])] PP[T];
			cout << "\n";
			delete[L] PP;
		} while (!printMap.end());
	}).detach();
...
void mainThreadCls::voegAdr(unsigned __int8 sess) {
	lock_guard<mutex> lk(mutChkAd);
	if (mpChkAd.find(sess) != mpChkAd.end()) mpChkAd.erase(sess);
}

void mainThreadCls::checkAdr(unsigned __int8 min) {
	if (min < 2) min = 60 - (2 - min); else min -= 2;
	blArdChk[0] = blArdChk[1] = 0; unsigned __int8 y;
	lock_guard<mutex> lk(mutChkAd);
	{
		bool dell = false; unsigned __int8 aa;
		for (map<unsigned __int8, __int8>::iterator it = mpChkAd.begin(); it != mpChkAd.end(); it++)
		{
			if (dell) dell = false, mpChkAd.erase(aa);
			if (++blArdChk[0] && (y = it->second) <= min || y > min + 2)
				ses.find(it->first)->second->closeF(), dell = true, aa = it->first, blArdChk[1]++;
...
void mainThreadCls::clsExMes::strExec::cancel() { 
	if (lsNow == 100) delete[32] ls; else point->retStr(lsNow); 
}

mainThreadCls::clsExMes::~clsExMes() {
	exCls.wait(); cvSend.wait(unique_lock<mutex>(sendMut), [&] { return lsT / 100; });
}


mainThreadCls::trLisHomeCls* mainThreadCls::sesPoint(mutex* mut, unsigned __int8 no) {
	if (no == 255) return NULL;
	mut->lock();
	shared_lock<shared_mutex> lk(threadCls->sMutSes);
	map<unsigned __int8, trLisHomeCls*>::iterator it; if ((it = ses.find(no)) != ses.end()) { mut->unlock(); return it->second; } mut->unlock(); return NULL;
}

unsigned __int8 mainThreadCls::adresNo(unsigned __int8 no) { shared_lock<shared_mutex> lk(sMutAdr); return adress[no]; }

void mainThreadCls::trAcc(SOCKET* s)
{
	{ lock_guard<mutex> lk(mutChkAd); mpChkAd.insert(pair<unsigned __int8, __int8>(accHome->mpSesNo.load(), p_clstijd->minuut.load())); blmutChkAd.store(true); }
...
bool mainThreadCls::newSes() {
	...
	for (unsigned __int8 t = 0; t < 255; t++) if (ses.find(t) == ses.end())
	{ ses.insert(pair<unsigned __int8, trLisHomeCls*>(accHome->mpSesNo = t, accHome)); return true; } } return false; 
}
	...
	for (unsigned int t = 0; t < len; t++)
		if ((thisChar = mes[t]) == '|') strLen = strAanv = 0;
			...
			if (thisChar == '+') //{
				...
				//}
				if (strAanv != 1) { 
					if (!timedMesF(strSndV{ mesStr, (int)strLen, exMes, exNow }))exCls->voeg(clsExMes::var{ mesStr, exNow, (unsigned __int8)strLen, (__int8)strAanv - 2 }); exNow = exMes->getStr(mesStr), strLen = strAanv = 0;
				}
				else {
					for (__int8 t = 0; t < 3; t++) if (t < 2) if (mesStr[t] < '0' || mesStr[t] > '9') break; else continue;
					else if (adrs.load() == 255)
					{
						threadCls->voegAdr(mpSesNo.load());
						adAd = ((mesStr[0] - '0') * 10) + (mesStr[1] - '0'); threadCls->sMutAdr.lock();
						if ((strAanv = threadCls->adress[adAd]) != 255) closesocket(threadCls->ses.find(strAanv)->second->socket);
		...
		else for (unsigned __int8 tt = 0; tt < 3; tt++) if (thisChar == aanv[tt]) strAanv = tt + 1;
}
...
void mainThreadCls::clsExMes::strExec::main() {
	...
	case 'D': switch (str[1]) { case 'B': {str[len] = '\0'; char* c[]{ "Debugbericht: ", str + 2 }; printTijdel(c, 2); } return; case 'M': return; } break;
...
void mainThreadCls::clsExMes::strExec::temp() {
	...
	for (unsigned __int8 t = 0, y = 0; (y = p.client[t]) != 255; t++) { zendId = threadCls->adresNo(strAd = y); strncpy_s(ls, 7, buf, 6); sendLen = 6; send(true); }
}

void mainThreadCls::killProc(char* x) {
...
void mainThreadCls::clsExMes::strExec::send(bool x) {
	unsigned __int8 ad2 = point->pLisHome->adrs.load();
...
__int8 mainThreadCls::clsExMes::getStr(char*& x) {
	{
		unique_lock<mutex> lk(sendMut);
		if (lsT) { __int8 y; x = lsP + ((y = lsA[--lsT]) * 32); 
		return y; }
	...
	if (p_clstijd->tijdupdate()) {
		{
			...
			if (p_clstijd->timeUpdC)
				for (unsigned __int8 t = 0, *p = p_clstijd->timeUpd, &m = p_clstijd->timeUpdC; t < m || (m = 0); t++)
...
void mainThreadCls::exMesOne::tijdUpdate2(char update, unsigned __int8 adress) {
...
void mainThreadCls::exMesOne::send() {
	if (bufX = threadCls->sesPoint(threadCls->sesPMut + zendId, zendId)) bufX->send(strSndV{ udpImplementation::stringPool->lPosF(ls, sendLen), sendLen * (-1) - 1 , NULL, lsNow }, 0);
	else udpImplementation::stringPool->ret(lsNow);
}
...
void mainThreadCls::clsExMes::strExec::stReq() {
	...
	unsigned __int8 theAd = point->pLisHome->adrs.load();
	{
		unsigned __int8 radAr[255], radCnt = 0;
		unsigned __int8 psBl;
		while (!strbasprc::vergCharP(strbasprc::remCommCharP(strbasprc::charPLsR(buf, fileObj.file, fileObj.loc)), "EE")) {
			...
			if (buf[0] == 'T' && (buf[1] == 'T' || buf[1] == 'E'))
				if (buf[1] == 'T') {
					lock_guard<mutex> lk(threadCls->timeMut);
					for (unsigned __int8 t = 0, &m = p_clstijd->timeUpdC, *p = p_clstijd->timeUpd; t < m || (p[m++] = strAd, 0); t++)
						if (p[t] == strAd) break;
					/*
					for (unsigned int t = 1; t > 0; t++)
						if (tijdsv.find(t - 1) == tijdsv.end()) {
							tijdsv.insert(pair<unsigned int, unsigned __int8>(t - 1, strAd)); break;
						}
						*/
				}
				else {
					__int8 temp = p_statvars->mptemp.find(strbasprc::cvintcharint(buf[4]))->second->temp.load();
					if (temp == 40) continue;
				...
				strncpy_s(buf, 11, "Apparaten\\", 10); buf[10] = ls[3]; buf[11] = ls[4];
				strncpy_s(buf + 12, 15, "\\Statusgevers\\", 14);
				strncpy_s(buf + 26, sendLen - 1, ls + 1, sendLen - 2); {
					unsigned __int8 bL = sendLen - 2 + 26; strncpy_s(buf + bL, 5, ".txt", 4); buf[bL + 4] = '\0';
				}
				fLock();
				if (!fileObj.open(buf)) { fUnlock(); continue; }
				if ([&](char* bUf) {
					if (bUf[0] != 'V' || bUf[1] != 'R') return false;
						for (unsigned __int8 t = (bUf[2] - '0') * 100 + (bUf[3] - '0') * 10 + (bUf[4] - '0'), T = 0; T < radCnt || (!(bUf = NULL) && (radAr[radCnt++] = t) && false); T++)
							if (radAr[T] == t) break;
				...
				if (!strbasprc::vergCharP(fileObj.file, "GG")) {
					strncpy_s(buf2, 11, "Apparaten\\", 10); char ad2[2] = { buf2[10] = fileObj.file[0], buf2[11] = fileObj.file[1] };
	...
	if (len != 5 || str[4] != 'R') { return; }
		...
		strncpy_s(buf2, 11, "Apparaten\\", 10); buf2[10] = buf[0]; buf2[11] = buf[1]; strncpy_s(buf2 + 12, 10, "\\Nummers\\", 9);
		...
		unsigned __int8 adT = point->pLisHome->adrs.load();
...
void mainThreadCls::clsExMes::strExec::readMes() {
	strncpy_s(buf, 11, "Apparaten\\", 10); buf[10] = str[2]; buf[11] = str[3]; strncpy_s(buf + 12, 11, "\\Signalen\\", 10); strncpy_s(buf + 22, len - 1, str + 2, len - 2); strncpy_s(buf + len + 20, 5, ".txt", 4);
...
strncpy_s(buf, 9, "Actie\'s\\", 8); strncpy_s(buf + 8, len + 1, str, len); strncpy_s(buf + 8 + len, 5, ".txt", 4);
	if (!fileObj.open(buf)) return;
	while (!strbasprc::vergCharP(strbasprc::remCommCharP(strbasprc::charPLsR(buf, fileObj.file, fileObj.loc)), "GG")) 
		if (buf[0] == 'N' && buf[1] == 'A') {
			...
			pExMp->voeg(var{ file, (__int8)zendId, bufX, -2 });
		}
		else {
...
bool mainThreadCls::clsExMes::strExec::passMes() {
	unsigned __int8 s_val = 0, s_type = 0; unsigned int s_no; strSndV s_s;
	if (len < 0) {
		len = (len + 1) * (-1);
		...
		strncpy_s(buf, 11, "Apparaten\\", 10); buf[10] = str[3]; buf[11] = str[4]; strncpy_s(buf + 12, 10, "\\Nummers\\", 9); strncpy_s(buf + 21, len - 3, str + 3, len - 4); strncpy_s(buf + 17 + len, 5, ".txt", 4); buf[23] = 'N'; buf[24] = 'O';
