...
void mainThreadCls::radioStr::open() {
	for (uint8_t t = 0; t < 100; t++) sendMpCh[t].ad = 100;
	strS = &threadCls->strCntS[LEDPL_CNT + 2].open(100, LEDPL_CNT + 2, 32);
	char buuuf[32];  memFileObj fileObj(fileData); uint8_t dat[255];
	...
	if (!chanCnt) return;
	passStatS = new passStatStr[chanCnt]; for (uint8_t t = 0; t < chanCnt; t++) passStatS[t].type = dat[t];
}

void mainThreadCls::radioStr::pass(uint16_t x, uint8_t ad) {
...
void mainThreadCls::radioStr::func(uint16_t x) {
	uint8_t X = x % 256;
...
void mainThreadCls::radioStr::passStatStr::pass(uint16_t x) {
...
void mainThreadCls::radioStr::passStatStr::func(uint16_t x) {
	memFileObj fileObj(fileData); 
	strncpy_s(ls, 7, "Radio/", 6);
	{ uint8_t no = this - radioS.passStatS; ls[6] = no / 100 + '0', ls[7] = (no / 10) % 10 + '0', ls[8] = no % 10 + '0'; }
	...
	while (strbasprc::charPLen(strbasprc::charPLsR(ls + 1, fileObj.file, fileObj.loc))) {
		uint8_t adSnd = (ls[3] - '0') * 10 + (ls[4] - '0'), b;
...
void mainThreadCls::radioStr::playPrep(uint8_t x, uint8_t old) {
	{ uint16_t X = x * 256 + old; if (!MpCha.begin(X)) return; }
	threadBufCls::recStruct{ radioS.threadMut, [](void* pMCh, void* p) {
		do ((mainThreadCls::radioStr*)p)->playF(((strMpCha*)pMCh)->p / 256, ((strMpCha*)pMCh)->p % 256); while (!((strMpCha*)pMCh)->end());
	} }.start(&MpCha, this);
}

void mainThreadCls::radioStr::playF(uint8_t x, uint8_t old) {
