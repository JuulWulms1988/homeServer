...
void mainThreadCls::trLisHomeCls::dimExF(mainThreadCls::strSndV s) {
	...
	else if ((str[4] == 'U' && str[5] == 'T' && (dimRicht = 3)) || (((str[4] >= '0' && str[4] <= '9') || (str[4] >= 'a' && str[4] <= 'f')) && ((((str[5] >= '0' && str[5] <= '9') || (str[5] >= 'a' && str[5] <= 'f')) && (dimRicht = 2)) || (str[5] >= 'A' && str[5] <= 'Q' && (dimRicht = 1) && (str[5] = [&](uint8_t XX) { if (XX < 10) return XX + '0'; else return (XX - 10) + 'a'; }(str[5] - 'A')))) && ((dimW = strbasprc::cv16waarde(str + 4)) || !dimW))) str[4] = 'A', str[5] = 'N';
	else return;
	strncpy_s(dimCBuf, 11, "Apparaten/11/Nummers/", 10); dimCBuf[10] = str[2]; dimCBuf[11] = str[3]; strncpy_s(dimCBuf + 12, 11, "/Signalen/", 10); strncpy_s(dimCBuf + 22, len - 1, str + 2, len - 2); strncpy_s(dimCBuf + len + 20, 5, ".txt", 4);
...
void mainThreadCls::trLisHomeCls::dimExAF(char* str, int len) {
	strncpy_s(dimCBuf, 9, "Actions/Namen", 8); strncpy_s(dimCBuf + 8, len + 1, str, len); strncpy_s(dimCBuf + 8 + len, 5, ".txt", 4);
	...
	while (!strbasprc::vergCharP(strbasprc::remCommCharP(strbasprc::charPLsR(str, file, loc)), "GG"))
		if (str[0] == 'N' && str[1] == 'A') {
			...
			for (uint8_t t = 0; t < len; t++) str[t] = str[t + 2];
				dimExAF(str, len);
		}
		else {
			...
			dimLsNow = exMes->getStr(dimLs); *dimLs = '@', strncpy_s(dimLs + 1, dimSendLen = strbasprc::charPLen(str) + 1, str, dimSendLen - 1), dimLs[2] = 'V', dimLs[dimSendLen++] = '+'; uint8_t aaa = adrs.load(); dimLs[dimSendLen] = aaa / 60 + '0', dimLs[dimSendLen + 1] = aaa % 60 + '0';
			dimCBufLen = (*strbasprc::charPLsR(dimCBuf, dimFileObj.file, dimFileObj.loc) - '1') * 100 + (dimCBuf[1] - '0') * 10 + (dimCBuf[2] - '0');
			threadCls->statStruc[900 + dimCBufLen].start(stSndStt{ (uint8_t)[&] { if ((!dimRicht && (dimLs[5] = 'a') && (dimLs[6] = 'n')) || (dimRicht == 3 && (dimLs[5] = 'u') && (dimLs[6] = 't'))) { len = (dimRicht / 3 + 1) % 2; return 7; } else { len = dimW; return dimRicht * 2 + 7; } }(), (unsigned int)dimCBufLen, (uint8_t)len, dimFileObj.loc, exMes, dimFileObj.file, strSndV{ dimLs, dimSendLen, exMes, dimLsNow } }, aaa);
		}
}

void mainThreadCls::stSndStt::funcD() {
	uint8_t w = threadCls->statDimVar[no].load();
